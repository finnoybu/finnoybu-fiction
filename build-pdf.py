"""
Build PDF for a Finnoybu trilogy book.

Assembles markdown chapters in order and runs Pandoc with xelatex.
Select which book to build via the BOOK env var (default 1).

Usage:
  python build-pdf.py            # Book I
  BOOK=2 python build-pdf.py     # Book II
  BOOK=3 python build-pdf.py     # Book III

Requires: pandoc, xelatex (MiKTeX or TeX Live), pyyaml
"""
...
def main():
    ...
    combined_md_path = os.path.join(OUTPUT_DIR, f"book-{BOOK}-combined-pdf.md")
    ...
    print("Running Pandoc with xelatex (this takes a minute or two)...")
    result = subprocess.run(pandoc_cmd, capture_output=True, text=True)

    if result.returncode != 0:
        print(f"  Pandoc STDERR: {result.stderr[:1000]}")
